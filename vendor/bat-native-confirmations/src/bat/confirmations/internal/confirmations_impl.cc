...
void ConfirmationsImpl::SetCatalogIssuers(std::unique_ptr<IssuersInfo> info) {
  DCHECK(state_has_loaded_);
  if (!state_has_loaded_) {
    BLOG(0, "Failed to set catalog issuers as Confirmations state is not "
        "ready");
    return;
  }
...
void ConfirmationsImpl::UpdateAdsRewards(const bool should_refresh) {
  DCHECK(state_has_loaded_);
  if (!state_has_loaded_) {
    BLOG(0, "Failed to update ads rewards as Confirmations state is not ready");
    return;
  }
...
void ConfirmationsImpl::GetTransactionHistory(
  ...
  DCHECK(state_has_loaded_);
  if (!state_has_loaded_) {
    BLOG(0, "Failed to get transaction history as Confirmations state is not "
        "ready");
    return;
  }
...
double ConfirmationsImpl::GetEstimatedRedemptionValue(
  ...
  if (it != catalog_issuers_.end()) {
    ...
    if (!re2::RE2::Replace(&name, "BAT", "")) {
      BLOG(0, "Failed to estimate redemption value due to invalid catalog "
          "issuer name");
    }
...
void ConfirmationsImpl::ConfirmAd(
    ...
    const ConfirmationType confirmation_type) {
  if (!state_has_loaded_) {
    BLOG(0, "Failed to confirm ad as Confirmations state is not ready");
    return;
  }
...
void ConfirmationsImpl::ConfirmAction(
  ...
  DCHECK(state_has_loaded_);
  if (!state_has_loaded_) {
    BLOG(0, "Failed to confirm action as Confirmations state is not ready");
    return;
  }
